...
from .laplacian_kernel import LaplacianKernel, postprocess_laplacian


class LaplacianKernelGrad(LaplacianKernel):
    r"""
    Computes a covariance matrix of the Laplacian kernel that models the covariance
    ...
    Example:
        ...
        >>> covar_module = gpytorch.kernels.ScaleKernel(gpytorch.kernels.LaplacianKernelGrad())
        ...
        >>> covar_module = gpytorch.kernels.ScaleKernel(gpytorch.kernels.LaplacianKernelGrad(batch_shape=torch.Size([2])))
    ...
    def forward(self, x1, x2, diag=False, **params):
        ...
        if not diag:
            ...
            # 1) Kernel block
            diff = self.covar_dist(x1_, x2_, square_dist=True, dist_postprocess_func=postprocess_laplacian, **params)
            K_11 = diff
            K[..., :n1, :n2] = K_11
            ...
            kp = KroneckerProductLazyTensor(
                torch.eye(d, d, device=x1.device, dtype=x1.dtype).repeat(*batch_shape, 1, 1) / self.lengthscale,
            ...
            kernel_diag = super(LaplacianKernel, self).forward(x1, x2, diag=True)
            grad_diag = torch.ones(*batch_shape, n2, d, device=x1.device, dtype=x1.dtype) / self.lengthscale
